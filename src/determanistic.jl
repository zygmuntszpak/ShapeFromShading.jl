...
WIP
"""
function retrieve_surface(algorithm::Union{Determanistic,Determanistic2}, startingShape::SynthShape, img::AbstractArray, integration_factor::Real = 10, smoothness::Real = 20; noiseVal::Real = 0, radius=50)
    # Generate initial gradient and dipatch on desired function.
    p,q = sythetic_gradient(startingShape, radius = radius, noise = noiseVal)
    p,q = retrieve_surface(algorithm, img, p, q, integration_factor, smoothness)
    return p,q
end

function retrieve_surface(algorithm::Determanistic, img::AbstractArray, pin::AbstractArray, qin::AbstractArray, integration_factor::Real = 10, smoothness::Real = 20)
...
function retrieve_surface(algorithm::Determanistic2, img::AbstractArray, pin::AbstractArray, qin::AbstractArray, integration_factor::Real = 10, smoothness::Real = 20)
    # Setup initial variables
    E = Float64.(img)
